...
def flatten_probas(probas, labels, ignore=None):
    ...
    if ignore is None:
        return probas, labels
    valid = (labels != ignore)
    # Use boolean indexing to preserve the class dimension even when only
    # a single valid element remains. Using nonzero().squeeze() can drop
    # dimensions and turn [P, C] into [C], which breaks later code that
    # expects dim==2. Boolean row indexing keeps shape [P_valid, C].
