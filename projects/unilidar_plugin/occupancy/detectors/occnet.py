...
class OccNet(BEVDepth):
    ...
    def forward_train(self,
        ...
        if self.random:
            points = self._sampling(points)
            # points = self.resample_lidar_point_cloud_torch(points)
    ...
    def simple_test(self, img_metas, img=None, points=None, voxel=None, dataset_flag=None, rescale=False, points_occ=None, 
        ...
        if isinstance(points, list):
            for i in range(len(points)):
                points[i] = points[i].to(torch.float32)
        # if self.random:
        #     points = self._sampling(points)
        if isinstance(gt_occ, list):        
           if all(isinstance(x, torch.Tensor) for x in gt_occ):
                for i in range(len(gt_occ)):
                    total_elements = gt_occ[i].shape[0]*gt_occ[i].shape[1]*gt_occ[i].shape[2]
                    if total_elements == 512 * 512 * 40:
                        gt_occ[i] = gt_occ[i].reshape(512, 512, 40)
