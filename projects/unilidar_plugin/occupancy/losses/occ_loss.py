...
Version: Do not edit
LastEditTime: 2025-07-15 06:18:41
Description: 头部注释
...
class OccLoss(nn.Module):
    ...
    def loss_voxel(self, output_voxels, target_voxels, tag, flag=None):
        ...
        if isinstance(target_voxels, list):        
            if all(isinstance(x, torch.Tensor) for x in target_voxels):
                for i in range(len(target_voxels)):
                    total_elements = target_voxels[i].shape[0]*target_voxels[i].shape[1]*target_voxels[i].shape[2]
                    if total_elements == 512 * 512 * 40:
                        target_voxels[i] = target_voxels[i].reshape(512, 512, 40)
                    elif total_elements == 256 * 256 * 32:
                        target_voxels[i] = target_voxels[i].reshape(256, 256, 32)
                    else:
                        # Handle unexpected sizes - either log a warning or use a default shape
                        print(f"Warning: Unexpected number of elements: {total_elements}")
                target_voxels = torch.stack(target_voxels, dim=0)
        if len(target_voxels.shape) != 4:
            target_voxels = target_voxels.reshape( B, 256, 256, -1)
        ...
        if not self.dual or flag is None:
        ...
        loss_dict['loss_voxel_ce_{}'.format(tag)] = torch.clamp(self.loss_voxel_ce_weight * CE_ssc_loss(output_voxels, target_voxels, class_weights.type_as(output_voxels), ignore_index=255), max=100)
        loss_dict['loss_voxel_sem_scal_{}'.format(tag)] = torch.clamp(self.loss_voxel_sem_scal_weight * sem_scal_loss(output_voxels, target_voxels, ignore_index=255), max=100)
        loss_dict['loss_voxel_geo_scal_{}'.format(tag)] = torch.clamp(self.loss_voxel_geo_scal_weight * geo_scal_loss(output_voxels, target_voxels, ignore_index=255, non_empty_idx=self.empty_idx), max=100)
        loss_dict['loss_voxel_lovasz_{}'.format(tag)] = torch.clamp(self.loss_voxel_lovasz_weight * lovasz_softmax(torch.softmax(output_voxels, dim=1), target_voxels, ignore=255), max=100)
    ...
    def loss_point(self, fine_coord, fine_output, target_voxels, tag):
        ...
        # igore 255 = ignore noise. we keep the loss bascward for the label=0 (free voxels)
        loss_dict['loss_voxel_ce_{}'.format(tag)] = torch.clamp(self.loss_voxel_ce_weight * CE_ssc_loss(fine_output, selected_gt, ignore_index=255), max=100)
        loss_dict['loss_voxel_sem_scal_{}'.format(tag)] = torch.clamp(self.loss_voxel_sem_scal_weight * sem_scal_loss(fine_output, selected_gt, ignore_index=255), max=100)
        loss_dict['loss_voxel_geo_scal_{}'.format(tag)] = torch.clamp(self.loss_voxel_geo_scal_weight * geo_scal_loss(fine_output, selected_gt, ignore_index=255, non_empty_idx=self.empty_idx), max=100)
        loss_dict['loss_voxel_lovasz_{}'.format(tag)] = torch.clamp(self.loss_voxel_lovasz_weight * lovasz_softmax(torch.softmax(fine_output, dim=1), selected_gt, ignore=255), max=100)
    ...
    def forward(self,
        ...
        if dataset_flag is None:
            ...
            for index, output_voxel in enumerate(output_voxels):
                res = self.loss_voxel(output_voxels= output_voxel, target_voxels=target_voxels,  tag='c_{}'.format(index))
                ...
                if self.sample_from_voxel or self.sample_from_img:
                    for index, (fine_coord, fine_output) in enumerate(zip(output_coords_fine, output_voxels_fine)):
                        ...
                        for k, v in this_batch_loss.items():
                            if this_batch_loss[k].numel() == 0:
                                continue
                            if k not in loss_batch_dict:
                                if torch.isnan(this_batch_loss[k]).any():
                                    continue
                                loss_batch_dict[k] = v
                            else:
                                if torch.isnan(this_batch_loss[k]).any():
                    ...
                    # 统计非空张量的数量
                    non_empty_count = sum(1 for tensor in output_coords_fine if tensor.numel() > 0)

                    # 使用非空张量的数量进行除法
                    if non_empty_count > 0:
                        for k, v in loss_batch_dict.items():
                            loss_dict[k] = v / non_empty_count
                    else:
                        for k, v in loss_batch_dict.items():
                            loss_dict[k] = 0     
                    
        
        elif not self.dual or (isinstance(dataset_flag, list) and len(dataset_flag) == 1):
            output_voxels = output['output_voxels']
            output_coords_fine = output['output_coords_fine']
            output_voxels_fine = output['output_voxels_fine']
            flag = dataset_flag[0]
