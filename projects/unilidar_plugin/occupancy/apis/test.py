...
def custom_multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False, show=False, out_dir=None):
    ...
    for i, data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            dataset_flag = data['dataset_flag'].data[0]
            if not hasattr(model.module, 'pts_bbox_head'):
                assert hasattr(model.module, 'tpv_aggregator') or hasattr(model.module, 'decode_head'), "Unknown Model Type"
                if hasattr(model.module, 'tpv_aggregator'):
                    if not model.module.tpv_aggregator.dual:
                        if show:
                            pred_f = result.get('pred_f')
                        ...
                        if 'SSC_metric_fine' in result.keys():
                            SSC_metric_fine.append(result['SSC_metric_fine'])
                elif hasattr(model.module, 'decode_head'):
                    if not model.module.decode_head.dual:
                        if show:
                            pred_f = result.get('pred_f')
                            save_occ(result['pred_c'], pred_f, data['img_metas'], out_dir, data['visible_mask'].data[0], data['gt_occ'].data[0],dataset_flag[0])
                    ...
                    if 'SSC_metric_fine_2' in result.keys():
                        SSC_metric_fine_2.append(result['SSC_metric_fine_2'])
            elif not model.module.pts_bbox_head.dual:
                if show:
                    pred_f = result.get('pred_f')
                    save_occ(result['pred_c'], pred_f, data['img_metas'], out_dir, data['visible_mask'].data[0], data['gt_occ'].data[0],dataset_flag[0])
    ...
    if 'SSC_metric' in result.keys():
        if SSC_metric is not None and len(SSC_metric) > 0 and all(tensor.shape == SSC_metric[0].shape for tensor in SSC_metric):
            ...
            SSC_metric = []
    SSC_metric_all = collect_results_cpu(SSC_metric, len(dataset), tmpdir)
    if rank == 0:
...
def collect_results_cpu(result_part, size, tmpdir=None, type='list'):
    ...
    if rank == 0:
        ...
        for i in range(world_size):
            ...
            if type == 'list':
                part_list.extend(list(res_i))
            else:
                raise NotImplementedError(
                    "collect_results_cpu currently only supports type='list'.")
