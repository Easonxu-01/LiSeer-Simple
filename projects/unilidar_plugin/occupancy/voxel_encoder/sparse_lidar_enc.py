...
Version: Do not edit
LastEditTime: 2024-12-10 08:30:57
Description: 头部注释
...
class SparseLiDAREnc8x(nn.Module):
    def __init__(self, input_channel, norm_cfg, base_channel, out_channel, 
        ...
        self.conv_input = spconv.SparseSequential(
            spconv.SubMConv3d(input_channel, base_channel, 3, algo=spconv.ConvAlgo.Native,),
            nn.GroupNorm(16, base_channel),
            nn.ReLU(inplace=True))
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        x = self.conv_out(x_conv3)
        pts_voxel_features = x.dense().permute(0,1,4,3,2)
        pts_feats = x.dense().permute(0,1,4,3,2)
        # return {'x': x.dense().permute(0,1,4,3,2), # B, C, W, H, D 
        #         'pts_feats': [x]}
        return pts_voxel_features, pts_feats
