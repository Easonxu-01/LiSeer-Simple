...
from projects.unilidar_plugin.occupancy.apis.test import custom_single_gpu_test, custom_multi_gpu_test
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs='?',
        default=None,
        help='checkpoint file (optional if config sets load_from)')
...
def main():
    ...
    assert args.out or args.eval or args.format_only or args.show \
        or args.show_dir or save_infer, \
        ('Please specify at least one operation (save/eval/format/show the '
         'results / save the results) with the argument "--out", "--eval"'
         ', "--format-only", "--show", "--show-dir", or set '
         'test_cfg.save_inference_preds=True in config')
    ...
    if checkpoint_path is None:
        raise ValueError(
            'No checkpoint: pass it as the 2nd positional argument, or set '
            '`load_from` in the config (e.g. pointtpv_realdata_infer.py).')
    ...
    if cfg.get('test_dual', False):
        ...
        datasets_sk = datasets_sk if isinstance(datasets_sk, (list, tuple)) else [datasets_sk]
        dataset = ConcatenatedDataset(datasets_nu, datasets_sk)
        data_loader = [
        build_dataloader(
            dataset,
        ...
        # shuffle=False so test / inference follows dataset order (e.g. list.txt),
        # and saved *.label files appear in sequence instead of sampler-shuffled order.
    ...
    if rank == 0 and distributed:
        ...
        if args.eval:
            if cfg.get('test_dual', False):
                eval_kwargs_sk = cfg.get('evaluation_sk', {}).copy()
                ...
                for key in [
                    ...
                    eval_kwargs_nu.pop(key, None)
                eval_kwargs_nu.update(dict(metric=args.eval, **kwargs))
                print(dataset.evaluate_nu(outputs, **eval_kwargs_nu))
            else:
