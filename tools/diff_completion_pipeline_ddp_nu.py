...
def inference(rank, diff, refine, denoising_steps, cond_weight, world_size):
    ...
    # Distribute samples among processes (each GPU handles part of the dataset)
    samples = nusc.sample
    samples_per_gpu = len(samples) // world_size
    start_idx = rank * samples_per_gpu
    end_idx = start_idx + samples_per_gpu if rank != world_size - 1 else len(samples)

    # Process assigned samples with tqdm progress bar
    for sample in tqdm.tqdm(samples[start_idx:end_idx], desc=f'Processing samples on GPU {rank}', position=rank):
        ...
        print(f'Sample {sample["token"]} on GPU {rank} took: {end - start}s')

        filename_without_extension = os.path.splitext(lidar_data['filename'])[0]
        new_filename = filename_without_extension + '.ply'
        # Define output paths
        output_refine_path = os.path.join('/home/eason/workspace_perception/LiDiff/lidiff/Datasets/nuScenes/samples/LIDAR_TOP/refine', new_filename)

        # Ensure output directories exist
        os.makedirs(os.path.dirname(output_refine_path), exist_ok=True)
