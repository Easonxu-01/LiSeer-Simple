...
def inference(rank, diff, refine, denoising_steps, cond_weight, world_size):
    ...
    # Path to SemanticKITTI dataset
    base_path = './Datasets/SemanticKITTI/dataset/sequences/'
    sequences = natsorted(os.listdir(base_path))

    # Distribute sequences among processes (each GPU handles part of the dataset)
    sequences_per_gpu = len(sequences) // world_size
    start_idx = rank * sequences_per_gpu
    end_idx = start_idx + sequences_per_gpu if rank != world_size - 1 else len(sequences)

    # Process assigned sequences
    for sequence in sequences[start_idx:end_idx]:
        sequence_path = os.path.join(base_path, sequence)
        velodyne_path = os.path.join(sequence_path, 'velodyne')
        output_refine_path = os.path.join(sequence_path, 'refine')
        output_diff_path = os.path.join(sequence_path, 'diff')

        # Ensure output directories exist
        os.makedirs(output_refine_path, exist_ok=True)
        os.makedirs(output_diff_path, exist_ok=True)

        # Process each .bin file in the velodyne folder
        for bin_file in tqdm.tqdm(natsorted(os.listdir(velodyne_path)), desc=f'Processing {sequence} on GPU {rank}'):
            ...
            start = time.time()
            refine_scan, diff_scan = model.module.complete_scan(points)  # Use model.module with DDP
            end = time.time()
            print(f'{bin_file} on GPU {rank} took: {end - start}s')
            ...
            diff_file = os.path.join(output_diff_path, bin_file.replace('.bin', '.ply'))
            o3d.io.write_point_cloud(diff_file, pcd_diff)
...
def main(diff, refine, denoising_steps, cond_weight, world_size):
    ...
    torch.multiprocessing.spawn(inference, args=(diff, refine, denoising_steps, cond_weight, world_size), nprocs=world_size)
